...
class MyCollectAdapter(context: Context) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view), View.OnClickListener {
        ...
        lateinit var currentEntity: VideoEntity

        init {
            view.setOnClickListener{
                mOnItemClickListener.onItemClick(currentEntity)
            }
                ...
                R.id.ll_collect -> {
                    ...
                    if (collectState) {
                        ...
                        if (collectNum > 0) {
                            ...
                            // 保存到数据库
                            updateCount(currentEntity.vid, 1, !collectState)
                        }
                    } else {
                        ++collectNum
                        tvCollect.text = collectNum.toString()
                ...
                R.id.ll_like -> {
                    ...
                    if (likeState) {
                        if (dzNum > 0) {
                            ...
                            updateCount(currentEntity.vid, 2, !likeState)
                        }
                    } else {
                        ++dzNum
                        tvDz.text = dzNum.toString()
